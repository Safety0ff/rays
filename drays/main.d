...
auto parseWorld(T)(T artFile) {
  sphere spheres[];
  string[] art;
  foreach (line; File(artFile).byLine) // can't use File.byLine.array because #6495
    art ~= line.idup;
...
auto genRays(uint canvasSize, ref seed_t seed, pixel px) pure nothrow
{
  immutable
...
auto raytraceWorld(W)(W world, ref seed_t seed, ReturnType!genRays rays) nothrow
{
...
void worker(W,R)(ubyte[3] result[], uint size, W world, R inputRange)
{
  ...
  // LDC generates crashing code for the following nested functions
  // DMD gives an error and rejects the code
  alias papply = curry;
  alias pixelToRays = papply!(papply!(genRays, size),seed);
  alias raysToSamples = papply!(papply!(raytraceWorld,world),seed);
  alias rayTrace = pipe!(pixelToRays,raysToSamples,samplesToColor,colorToRGB24);
  size_t i = 0;
  for (auto rng = inputRange; !rng.empty; rng.popFront(), ++i)
  {
    auto pixel = rng.front;
    result[i] = rayTrace(pixel);
  }
}

auto bindWorkerArgs(W,R)(ubyte[3] result[], uint size, W world, R inputRange)
{ // Because a lambda in a loop doesn't seem to create a new context for each iteration
